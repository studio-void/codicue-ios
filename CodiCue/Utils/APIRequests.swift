...
func sendPostRequest<T: Encodable>(
    endpoint: String,
    parameters: T,
    ...
    let updatedHeaders =
        headers != nil
        ? HTTPHeaders(
            defaultHeaders.dictionary.merging(headers!.dictionary) { $1 }
        ) : defaultHeaders
    ...
    print(
        "Response Data: \(String(data: response.data ?? Data(), encoding: .utf8) ?? "No Data")"
    )
